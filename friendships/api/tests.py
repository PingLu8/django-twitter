...
class FriendshipApiTests(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(2):
            follower = self.create_user(f'dongxie_follower{i}')
            Friendship.objects.create(from_user=follower, to_user=self.dongxie)
        for i in range(3):
            following = self.create_user(f'dongxie_following{i}')
    ...
    def test_follow(self):
        ...
        # follow again, duplicate without an error
        response = self.dongxie_client.post(url)
        self.assertEqual(response.status_code,201)
        self.assertEqual(response.data['duplicate'], True)

        # follow back, success. increase friendship count
